...
- (void)viewDidLoad {
    ...
    CGFloat topPos = kStatusBarAndNavigationBarHeight + 40;
    ...
    self.loginBtn = [[UIButton alloc] initWithFrame:CGRectMake(paddingEdge, topPos + 10 + 35, bgRect.size.width - paddingEdge - paddingEdge, fieldHeight)];
...
- (void)changeLanage {
    ...
    //初始化一个UIAlertController的警告框将要用到的UIAlertAction style Default
    UIAlertAction *yuenan = [UIAlertAction actionWithTitle:@"Tiếng Việt" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
...
- (void)addBtn {
    ...
    [btn setTitleColor:UIColor.whiteColor forState:UIControlStateSelected];
    btn.titleLabel.font = [UIFont boldSystemFontOfSize:14];
    [btn setTitle:@"Tiếng Việt" forState:UIControlStateNormal];
    btn.tag = 1000 + 3;
    [btn addTarget:self action:@selector(setLanguageClick:) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:btn];
    [a addObject:btn];
