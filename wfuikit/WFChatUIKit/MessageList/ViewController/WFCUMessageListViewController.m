...
static SystemSoundID redBagSoundID = 0;
static SystemSoundID normalSoundID = 0;
...
@property (nonatomic, strong)WFCUMessageCellBase *redBagCell;
@end
...
//            hud.offset = CGPointMake(0.f, MBProgressMaxOffset);
    ...
    self.collectionView.backgroundColor = WFCCUtilities.backgoundColor;
}
...
- (void)onRightBarBtn:(UIBarButtonItem *)sender {
    ...
    .LeeTitle(WFCString(@"SearchGame"))
    .LeeContent(WFCString(@"InputGameNumber"))
    .LeeAddTextField(^(UITextField * _Nonnull textField) {
        textField.placeholder = WFCString(@"GameNumber");
        _searchField = textField;
    }).LeeCancelAction(WFCString(@"Cancel"), ^{
        
    }).LeeAction(WFCString(@"Ok"), ^ {
...
- (void)initializedSubViews {
    ...
    [self registerCell:[AgroupLuckyMoneyAccept class] forContent:[AgroupLuckyMoneyAcceptMessageContent class]];
    [self registerCell:[AgroupLuckyMoneyCell class] forContent:[AgroupLuckyMoneyMessageContent class]];
    
    [self registerCell:[SingleLuckyMoneyAxcepted class] forContent:[LuckyMoneyAcceptMessageContent class]];
    [self registerCell:[WFCLuckMoneyCell class] forContent:[WFCCLuckMoneyMessage class]];
    [self registerCell:[WFCTransferMoneyCell  class] forContent:[WFCCTransferMoneyMessage class]];
    [self registerCell:[TransferMoneyAcceptedCell class] forContent:[TransferMoneyAcceptModel class]];
...
- (void)appendMessages:(NSArray<WFCCMessage *> *)messages newMessage:(BOOL)newMessage highlightId:(long)highlightId {
    ...
    for (int i = 0; i < messages.count; i++) {
        ...
        if ([LuckyManager filterMessage:message]) {
            continue;
        }
        ...
        if (newMessage) {
            BOOL showTime = YES;
            if (self.modelList.count > 0 && (message.serverTime -  (self.modelList[self.modelList.count - 1]).message.serverTime < 10 * 1000)) {
                showTime = NO;
            }
            ...
            [self autoShowGroupLuckyMoney:message];
        } else {
            if (self.modelList.count > 0 && (self.modelList[0].message.serverTime - message.serverTime < 10 * 1000) && i != 0) {
...
- (void)autoShowGroupLuckyMoney:(WFCCMessage *)message {
    
    NSString *userID = [[WFCCNetworkService sharedInstance] userId];
//    if (message.fromUser == nil || [message.fromUser isEqualToString: userID]) {
//        return;
//    }
    BOOL slient = UserInfo.sharedInstance.isSlient;
    if (!slient) {
        if ([message.content isKindOfClass:[AgroupLuckyMoneyMessageContent class]]) {
            /// red bag voice
            if (redBagSoundID == 0) {
                NSString *path = [[NSBundle bundleForClass:self.class] pathForResource:@"redbag" ofType:@"mp3"];
                //定义一个SystemSoundID
                ...
                redBagSoundID = soundID;
            }
            if (redBagSoundID != 0) {
                //播放声音
                AudioServicesPlaySystemSound(redBagSoundID);
            }
        } else {
            /// normal message
            if (redBagSoundID == 0) {
                NSString *path = [[NSBundle bundleForClass:self.class] pathForResource:@"normal" ofType:@"m4a"];
                //定义一个SystemSoundID
                SystemSoundID soundID;
                //判断路径是否存在
                if (path) {
    ...
    if (dTime - message.serverTime > 1000 * 10) {
        return;
    }
    LuckyManager *lManager = [[LuckyManager alloc] init];
    lManager.userId = [[WFCCNetworkService sharedInstance] userId];
    lManager.message = message;
    lManager.conversation = self.conversation;
    lManager.vc = self;
    [self.chatInputBar resetInputBarStatue];
    [lManager showLuckyMoneyMessage];
}
...
#pragma mark - MessageCellDelegate 点击消息时动作调用函数
- (void)didTapMessageCell:(WFCUMessageCellBase *)cell withModel:(WFCUMessageModel *)model {
    ...
    } else if ([model.message.content isKindOfClass:[WFCCTransferMoneyMessage class]]) {
        MessageContentModel *cModel = [MessageContentModel messageWithJson:model.message.content.extra];
        #pragma mark - 点击转账Cell
        if (model.message.direction == MessageDirection_Receive) {
           /// 接收者
            TransferMoneyDetailViewController *tvc = [[TransferMoneyDetailViewController alloc] init];
            tvc.redPId = cModel.redPId;
            tvc.userId = [[WFCCNetworkService sharedInstance] userId];
            tvc.message = model.message;
            __weak typeof(self) weakSelf = self;
            tvc.accept = ^{
                [weakSelf acceptTransferMoney:cModel originMessage:model.message];
            };
    ...
    } else if ([model.message.content isKindOfClass:[TransferMoneyAcceptModel class]]) {
        MessageContentModel *cModel = [MessageContentModel messageWithJson:model.message.content.extra];
        TransferMoneyDetailViewController *tvc = [[TransferMoneyDetailViewController alloc] init];
        tvc.isSender = YES;
        tvc.redPId = cModel.redPId;
        [self presentViewController:tvc animated:YES completion:nil];
    } else if ([model.message.content isKindOfClass:[WFCCLuckMoneyMessage class]]) {
        [self.chatInputBar resetInputBarStatue];
        LuckyManager *lManager = [[LuckyManager alloc] init];
        lManager.userId = [[WFCCNetworkService sharedInstance] userId];
        lManager.message = model.message;
        lManager.conversation = self.conversation;
        lManager.vc = self;
        [lManager showLuckyMoneyMessage];
    } else if ([model.message.content isKindOfClass:[AgroupLuckyMoneyMessageContent class]]) {
        [self.chatInputBar resetInputBarStatue];
        /// group_LuckyMoney
        LuckyManager *lManager = [[LuckyManager alloc] init];
        lManager.userId = [[WFCCNetworkService sharedInstance] userId];
 ...
 */
- (void)showTransferDetail:(long)messageId {
...
- (void)videoDidCapture:(NSString *)videoPath thumbnail:(UIImage *)image duration:(long)duration {
    ...
    WFCCVideoMessageContent *videoContent = [WFCCVideoMessageContent contentPath:videoPath thumbnail:image duration:format_time];
...
- (void)didSelectLuckyMoney {
    if(self.conversation.type == Single_Type) {
        StartSendLuckyMoneyViewController *svc = [StartSendLuckyMoneyViewController new];
        svc.targetUser = self.targetUser;
        [self presentViewController:svc animated:YES completion:nil];
        __weak typeof(self) weakSelf = self;
        svc.block = ^(NSString * _Nonnull content) {
            //发送消息时，client会发出"kSendingMessageStatusUpdated“的通知，消息界面收到通知后加入到列表中。
            MessageContentModel *mContent = [MessageContentModel messageWithJson:content];
            NSString *messageContent = [NSString stringWithFormat:@"[%@红包]%@",kTargetName,mContent.note];
            WFCCLuckMoneyMessage *lMessage = [WFCCLuckMoneyMessage contentWith:messageContent];
            lMessage.extra = content;
            //发送消息时，client会发出"kSendingMessageStatusUpdated“的通知，消息界面收到通知后加入到列表中。
            NSMutableArray *tousers = nil;
            if (self.privateChatUser) {
                tousers = [[NSMutableArray alloc] init];
            ...
            [[WFCCIMService sharedWFCIMService] send:weakSelf.conversation content:lMessage toUsers:tousers expireDuration:0 success:^(long long messageUid, long long timestamp) {
                /// 发送成功的回调
            } error:^(int error_code) {
            }];
        };
    } else if (self.conversation.type == Group_Type) {
        StartSendGroupLuckyMoneyController *svc = [StartSendGroupLuckyMoneyController new];
        svc.agroupId = self.targetGroup.target;
        [self presentViewController:svc animated:YES completion:nil];
        __weak typeof(self) weakSelf = self;
        svc.block = ^(NSString * _Nonnull content) {
            //发送消息时，client会发出"kSendingMessageStatusUpdated“的通知，消息界面收到通知后加入到列表中。
            MessageContentModel *mContent = [MessageContentModel messageWithJson:content];
            NSString *messageContent = [NSString stringWithFormat:@"[%@红包]%@",kTargetName,mContent.note];
            AgroupLuckyMoneyMessageContent *lMessage = [AgroupLuckyMoneyMessageContent contentWith:messageContent];
            lMessage.extra = content;
            //发送消息时，client会发出"kSendingMessageStatusUpdated“的通知，消息界面收到通知后加入到列表中。
            NSMutableArray *tousers = nil;
            if (self.privateChatUser) {
...
/**
 发起转账
 */
- (void)didSelectTransferAccounts {
    
    if(self.conversation.type == Single_Type) {
        StartTransferMoneyViewController *svc = [StartTransferMoneyViewController new];
        svc.targetUser = self.targetUser;
        [self presentViewController:svc animated:YES completion:nil];
        __weak typeof(self) weakSelf = self;
        svc.block = ^(NSString * _Nonnull content) {
            MessageContentModel *model = [MessageContentModel messageWithJson:content];
#warning 需要转为json之后在赋值
            NSString *mContent = [NSString stringWithFormat:@"[%@转账]%@",kTargetName,model.note];
            WFCCTransferMoneyMessage *lMessage = [WFCCTransferMoneyMessage contentWith:mContent];
            lMessage.extra = content;
            //发送消息时，client会发出"kSendingMessageStatusUpdated“的通知，消息界面收到通知后加入到列表中。
            NSMutableArray *tousers = nil;
            if (self.privateChatUser) {
...
/**
 ...
 @param model 转账的消息内容
 */
- (void)acceptTransferMoney:(MessageContentModel *)model originMessage:(WFCCMessage *)oMessage {
    TransferMoneyAcceptModel *lMessage = [TransferMoneyAcceptModel contentWith:@"接收转账"];
    //发送消息时，client会发出"kSendingMessageStatusUpdated“的通知，消息界面收到通知后加入到列表中。
    __weak typeof(self) ws = self;
    NSMutableArray *tousers = nil;
    if (self.privateChatUser) {
    ...
    MessageContentModel *m = [MessageContentModel messageWithJson:oMessage.content.extra];
    m.messageUid = oMessage.messageUid;
    m.status = 1;
    m.type = 1;
    NSString *update = m.mj_JSONString;
    oMessage.content.extra = update;
    [MessageContentModel insertMessage:m];
    lMessage.extra = update;
    [[WFCCIMService sharedWFCIMService] send:self.conversation content:lMessage toUsers:tousers expireDuration:0 success:^(long long messageUid, long long timestamp) {
        /// 发送成功的回调
    } error:^(int error_code) {
    }];
}
...
/**
 红包接受之后发送接收到的消息
 @param content 点击的Messsage
 */
- (void)acceptLuckyMoneySuccess:(NSString *)extra {
    MessageContentModel *m = [MessageContentModel messageWithJson:extra];
    if (m.type == 3) {
        AgroupLuckyMoneyAcceptMessageContent *acceptModel = [AgroupLuckyMoneyAcceptMessageContent contentWith:@"领取红包"];
        acceptModel.extra = extra;
        [self sendMessage:acceptModel];
        return;
    }
    LuckyMoneyAcceptMessageContent *acceptModel = [LuckyMoneyAcceptMessageContent contentWith:@"领取红包"];
    acceptModel.extra = extra;
    [self sendMessage:acceptModel];
}
...
- (void)reloadMessage {
    if (self.redBagCell) {
        [self.collectionView reloadItemsAtIndexPaths:@[[self.collectionView indexPathForCell:self.redBagCell]]];
        self.redBagCell = nil;
    }
}
...
- (void)displayMenu:(WFCUMessageCellBase *)baseCell {
    ...
    if (baseCell.model.message.direction == MessageDirection_Receive && !([baseCell isKindOfClass:[WFCLuckMoneyCell class]] || [baseCell isKindOfClass:[WFCTransferMoneyCell class]])) {
...
//        [items addObject:forwardItem];
    ...
    /// 如果为红包和转账不能进行撤回
    if ([baseCell isKindOfClass:[WFCLuckMoneyCell class]] || [baseCell isKindOfClass:[WFCTransferMoneyCell class]]) {
        canRecall = NO;
    }
...
- (void)search:(NSString *)groupId {
    [[NetworkAPI sharedInstance] v2GetWithUrl:@"/app/game" params:@{@"id":groupId} successComplection:^(NSDictionary * _Nonnull done) {
        if (done[@"data"] && [done[@"data"] isKindOfClass:NSDictionary.class]) {
            [[NSNotificationCenter defaultCenter] postNotificationName:@"kSearchResultNotificationName" object:self userInfo:done[@"data"]];
        } else {
